...
module.exports = function (options) {
  ...
  Chart.prototype.updateAxes = function () {
    ...
    if (this.options.conj[0].dom == 'Numer'){
      ...
      .scale(xScale)
      .tickSize(options.grid ? -height : 0)
      .tickFormat(function(d){if (d == Math.round(d) && 0 <= d) { return valores1[Math.abs(parseInt(d, 10))];}})
      .orient('bottom')
    ...
    if (this.options.conj[0].cod == 'Numer'){
      ...
      if (this.options.conj[0].baseCod == 'Z'){
        ...
        .scale(yScale)
        .tickSize(options.grid ? -width : 0)
        .tickFormat(function(d) {if (d == Math.round(d) && fun3(d)){ return d }})
        .orient('left')
      } else  { 
        this.meta.yAxis = d3.svg.axis()
        .scale(yScale)
        .tickSize(options.grid ? -width : 0)
        .tickFormat(function(d) {if (fun3(d)){ return d }})
        .orient('left')
      }
  ...
  Chart.prototype.setUpEventListeners = function () {
    ...
    var events = {
      ...
      /*if (this.options.conj[0].dom =='Func') {
        var fun = this.options.conj[0].sets.fdom;
        if ( this.options.conj[0].baseDom != 'R'){
      ...
      }else if (this.options.conj[0].dom =='Numer' || this.options.conj[0].dom =='Z' || this.options.conj[0].dom =='N'){  //Cambio 1 de agosto
          var xRound = Math.round(coordinates.x)
          if(this.options.conj[0].dom =='N'){
               if(xRound >= 0){
                    coordinates.x = xRound 
                    instance.tip.move(coordinates, instance.options.tip.color)
                } 
          }else if(this.options.conj[0].dom =='Numer'){
                if(xRound >= 0 && this.options.conj[0].sets.fdom.length >= xRound){
                  coordinates.x = xRound 
                  instance.tip.move(coordinates, instance.options.tip.color)
                } 
          }else{
