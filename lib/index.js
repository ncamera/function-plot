...
var builtInEvaluator = require('./helpers/eval').builtIn//YO 20-Agosto
...
module.exports = function (options) {
  ...
  Chart.prototype.internalVars = function () {
    ...
    this.meta.zoomBehavior = d3.behavior.zoom()
    if (this.options.zoom){
      var zoo = this.options.zoom;
      zoo = 8 / zoo;
    ...
    zoomBehavior = this.meta.zoomBehavior;
    ...
    // Set axis value
    if (!this.options.hasOwnProperty('axis') || this.options.axis === true) {
      this.options.axis = true
    }
  ...
  Chart.prototype.buildAxisLabel = function () {
    ...
    if (!this.options.axis) {
      canvas.selectAll('text.x.axis-label').remove()
      canvas.selectAll('text.y.axis-label').remove()
    } else {
      // axis labeling
      var xLabel, yLabel
  ...
  Chart.prototype.updateAxes = function () {
    ...
    if (this.options.conj[0].dom == 'Numer'){
      var valores1 = this.options.conj[0].sets.fdom;
      for (var i = 0; i < valores1.length - 1; i += 1) { 
        if (valores1[i].length > 10){
          valores1[i] = valores1[i].substr(0,7)+"..."
        }
      }
      var cant = (valores1.length / 2);
      this.meta.xAxis = d3.svg.axis()
      .scale(xScale)
      .tickSize(options.grid ? -height : 0)
      .tickFormat(!this.options.axis ? "" : function(d){if (d == Math.round(d) && 0 <= d) { return valores1[Math.abs(parseInt(d, 10))];}})
      .orient('bottom')
    }else if(this.options.conj[0].dom == 'Func'){
      var fun2 = this.options.conj[0].sets.fdom;
      if(this.options.conj[0].baseDom == 'Z'){
         this.meta.xAxis = d3.svg.axis()
         .scale(xScale)
         .tickSize(options.grid ? -height : 0)
         .tickFormat(!this.options.axis ? "" : function(d) {if (d == Math.round(d) && fun2(d)){ return d }})
         .orient('bottom')
    //  }else{
    //    this.meta.xAxis = d3.svg.axis()
    //    .scale(xScale)
    ...
    }else if(this.options.conj[0].dom == 'Z'){
      this.meta.xAxis = d3.svg.axis()
      .scale(xScale)
      .tickSize(options.grid ? -height : 0)
      .tickFormat(!this.options.axis ? "" : function(d) {if (d == Math.round(d)){ return parseInt(d, 10) }})
      .orient('bottom')
    }  

    ///Eje y
    if (this.options.conj[0].cod == 'Numer'){
      var valores = this.options.conj[0].sets.fcod;
      for (var j = 0; j < valores.length - 1; j += 1) { 
        if (valores[j].length > 10){
          valores[j] = valores[j].substr(0,7)+"..."
        }
      }
      this.meta.yAxis = d3.svg.axis()
      .scale(yScale)
      .tickSize(options.grid ? -width : 0)
      .tickFormat(!this.options.axis ? "" : function(d) {if (d == Math.round(d) && 0 <= d) { return  valores[Math.abs(parseInt(d, 10))];}})
      .orient('left')
    }else if(this.options.conj[0].cod == 'Func'){
      var fun3 = this.options.conj[0].sets.fcod;
      if (this.options.conj[0].baseCod == 'Z'){
        this.meta.yAxis = d3.svg.axis()
        .scale(yScale)
        .tickSize(options.grid ? -width : 0)
        .tickFormat(!this.options.axis ? "" : function(d) {if (d == Math.round(d) && fun3(d)){ return d }})
        .orient('left')
   //   } else  { 
   //     this.meta.yAxis = d3.svg.axis()
    //    .scale(yScale)
   //     .tickSize(options.grid ? -width : 0)
    //    .tickFormat(!this.options.axis ? "" : function(d) {if (fun3(d)){ return d }})
    //    .orient('left')
      }
    }else if (this.options.conj[0].cod == 'Z'){
      this.meta.yAxis = d3.svg.axis()
      .scale(yScale)
      .tickSize(options.grid ? -width : 0)
      .tickFormat(!this.options.axis ? "" : function(d) {if (d == Math.round(d)){ return parseInt(d, 10) }})
  ...
  Chart.prototype.toggleGrid = function() {
    var instance = this
    var grid = instance.options.hasOwnProperty('grid') && instance.options.grid === true;
    instance.options.grid = !grid;
    instance.build()
    instance.draw()
  }

  Chart.prototype.toggleAxis = function() {
     var instance = this
     var axis = !instance.options.hasOwnProperty('axis') || instance.options.axis === true
     instance.options.axis = !axis
     instance.build()
     instance.draw()
  }

  Chart.prototype.toggleTip = function() {
    var instance = this
    var skipTip = instance.options.hasOwnProperty('skipTip') && instance.options.skipTip === true
    instance.options.skipTip = !skipTip
    instance.options.data.forEach(function callback(chart, index, array) {
      chart.skipTip = instance.options.skipTip
    })
    if (instance.options.skipTip) {
  ...
  Chart.prototype.removeAllGraphs = function () {
    cache[instance.id].options.data = [];
  ...
  Chart.prototype.setUpEventListeners = function () {
    ...
    var events = {
      mousemove: function (coordinates) {
        ...
        var i;
        //console.log(instance.options.data.length)
        instance.tip.move(coordinates,false,instance)//instance.options.tip.color)
